    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in stud.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer stud.* over opencode.*, prefer .jsonc over .json)
  const candidates = [
    path.join(baseDir, "stud.json"),
    path.join(baseDir, "stud.jsonc"),
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".stud", "stud.json"),
      path.join(baseDir, ".stud", "stud.jsonc"),
  ...
  // Default to stud.json if none exist
  return candidates[0]
}
