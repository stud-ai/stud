...
const TIPS = [
  ...
  "Create {highlight}stud.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/stud/stud.json{/highlight} for global config",
  ...
  "Stud auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.stud/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.stud/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Stud auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "Stud uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.stud/tool/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.stud/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Stud from reading sensitive files",
  "Use {highlight}stud run{/highlight} for non-interactive scripting",
  "Use {highlight}stud --continue{/highlight} to resume the last session",
  "Use {highlight}stud run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}stud serve{/highlight} for headless API access to Stud",
  "Use {highlight}stud run --attach{/highlight} to connect to a running server",
  "Run {highlight}stud upgrade{/highlight} to update to the latest version",
  "Run {highlight}stud auth list{/highlight} to see all configured providers",
  "Run {highlight}stud agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.stud/themes/{/highlight} directory",
  ...
  "Run {highlight}stud debug config{/highlight} to troubleshoot configuration",
